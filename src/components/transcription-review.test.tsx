...
const REVIEW: TranscriptionReview = {
  ...
  events: [
    { index: 0, pitch_concert_midi: 60, written_pitch_midi: 69, onset_seconds: 0, offset_seconds: 0.5, velocity: 90, confidence: 0.42, is_low_confidence: true },
    { index: 1, pitch_concert_midi: 67, written_pitch_midi: 76, onset_seconds: 0.25, offset_seconds: 1, velocity: 100, confidence: 0.82, is_low_confidence: false },
  ],
};
...
describe("TranscriptionReviewView", () => {
  it("renders loading then complete semantic table and timeline", async () => {
    ...
    expect(screen.getByText("Confidence is a model signal, not calibrated accuracy.")).toBeVisible();
    ...
    expect(within(rows[1]).getByText("0.5 s")).toBeVisible();
  ...
  it("distinguishes an empty successful result from not ready", async () => {
    renderReview(
      vi.fn().mockResolvedValue({ ...REVIEW, summary: { event_count: 0, low_confidence_count: 0 }, events: [] }),
    );
    expect(await screen.findByText("No note events were produced.")).toBeVisible();
    expect(screen.queryByText("Transcription notes are not available yet.")).not.toBeInTheDocument();
  });
  ...
  it.each([
    ...
    renderReview(vi.fn().mockRejectedValue(new TranscriptionApiError(code, message, "job_id", status)));
  ...
  it("does not expose editing playback or polling controls", async () => {
    ...
    expect(screen.queryByRole("button", { name: /edit|delete|play|pause|refresh/i })).not.toBeInTheDocument();
  });
});
